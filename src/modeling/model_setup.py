...
model_name = f"lichess_stockfish_mix_8layers_ckpt_no_optimizer.pt"
...
if LOAD_AND_CONVERT_CHECKPOINT:
    ...
    torch.save(
        convert_nanogpt_weights(synthetic_checkpoint, cfg),
        f"{MODEL_DIR}tf_lens_{recorded_model_name}.pth",
    )
    torch.save(cfg.to_dict(), f"{MODEL_DIR}tf_lens_{recorded_model_name}_cfg.pth")
